...
<div class="row">
    <div class="col-md-8">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('orb-ownings.update', $orbOwning) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <select id="account_id" name="account_id" class="form-control" required>
                            @foreach($accounts as $account)
                                <option value="{{ $account->id }}" {{ $orbOwning->account_id == $account->id ? 'selected' : '' }}>{{ $account->account_name }}</option>
                            @endforeach
                        </select>
                        @error('account_id')
                    ...
                    <div class="form-group position-relative">
                        ...
                        <input type="hidden" id="dragon_id" name="dragon_id" value="{{ old('dragon_id', $orbOwning->dragon_id) }}">
                        <input type="text" id="dragon_name" name="dragon_name" class="form-control" placeholder="Type at least 3 characters..." value="{{ old('dragon_name', $orbOwning->dragon->dragon_name) }}" autocomplete="off" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="jumlah_orb" name="jumlah_orb" class="form-control" min="0" value="{{ old('jumlah_orb', $orbOwning->jumlah_orb) }}" required>
                        @error('jumlah_orb')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
