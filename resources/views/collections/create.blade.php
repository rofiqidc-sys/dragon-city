            ...
            <div class="card-body">
                <form action="{{ route('collections.store') }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="collection_name" id="collection_name" class="form-control @error('collection_name') is-invalid @enderror" value="{{ old('collection_name') }}" required>
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="number" name="gem_reward" id="gem_reward" class="form-control @error('gem_reward') is-invalid @enderror" value="{{ old('gem_reward', 0) }}" min="0" required>
                            @error('gem_reward')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group col-md-6">
                            ...
                            <select name="dragon_reward_id" id="dragon_reward_id" class="form-control @error('dragon_reward_id') is-invalid @enderror">
                                ...
                                @foreach($dragons as $dragon)
                                    <option value="{{ $dragon->id }}" @if(old('dragon_reward_id') == $dragon->id) selected @endif>{{ $dragon->dragon_name }} ({{ $dragon->rarity->name ?? '-' }})</option>
                                @endforeach
                            </select>
